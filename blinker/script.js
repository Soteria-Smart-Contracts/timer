document.addEventListener('DOMContentLoaded', () => {
    ...
    let treeStates = Array.from({ length: plots.length }, () => ({})); // Initialize treeStates as an array
    ...
    chrome.storage.local.get(['treeStates', 'totalBlinkersToday', 'highScore'], ({ treeStates: ts, totalBlinkersToday: tb, highScore: hs }) => {
        treeStates = ts || Array.from({ length: plots.length }, () => ({}));
    ...
    function resetDailyCountAtMidnight() {
        ...
        setTimeout(() => {
            ...
            treeStates = Array.from({ length: plots.length }, () => ({}));
            chrome.storage.local.set({ treeStates, totalBlinkersToday, highScore }, () => {
    ...
    plots.forEach((plot, index) => {
        plot.addEventListener('click', () => {
            if (!isBlinking && (!treeStates[index].planted || treeStates[index].dead)) {
                isBlinking = true;
